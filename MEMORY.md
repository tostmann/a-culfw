...
*   **XIAO-ESP32-C3 Boot-Problem mit CC1101 (Benötigt manuelle Intervention):**
    ...
    *   **Status des Build-Systems (AI-Fehlschlag):** Die AI scheiterte **wiederholt und hartnäckig** daran, die Pin-Konfiguration und die Build-Pfade (extra_scripts, include-Pfade, src_dir, build_src_filter) in der `platformio.ini` automatisch zu aktualisieren und das Image zu bauen. Trotz zahlreicher Korrekturversuche, die relative/absolute Pfade für Skripte, `src_dir` und `build_src_filter` betrafen, traten immer wieder Probleme auf: `pio: not found`, `NotPlatformIOProjectError`, `missing SConscript file`, `Error: Nothing to build. Please put your source code files to the '.../src' folder`, und schließlich `fatal error: hal_gpio.h: No such file or directory`. Nach mehrfacher Überschreitung der Ausführungslimits und Versuchen, die Build-Struktur anzupassen (inkl. temporärer `src`-Verzeichnisse), wurde die `platformio.ini` zuletzt auf den ursprünglichen Zustand (`git checkout`) zurückgesetzt. Dies bedeutet, dass das XIAO-ESP32-C3 Target derzeit in der `platformio.ini` noch das inkorrekte, nicht-sichere Pinout verwendet und die AI es nicht geschafft hat, einen erfolgreichen Build zu erzielen. Manuelle Korrektur ist nach wie vor unerlässlich.
