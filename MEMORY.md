...
*   **RF-Datenaustausch (Slow RF - FS20, Intertechno):**
    *   **Problem:** Die Hardware (XIAO-ESP32-C3) erkennt erfolgreich Sync-Sequenzen (`S`) und wechselt in den `STATE_COLLECT`-Modus (`C`). Die Debug-Ausgabe zeigt `C<hightime/lowtime>.R` (z.B. `C<403/412>.R`), was bedeutet, dass ein Paket-Sync erkannt und die Bit-Sammlung gestartet wurde, aber unmittelbar danach ein Reset (`R`) erfolgt. Dies deutet darauf hin, dass das erste oder eines der ersten Datenbits nach dem Sync von der `wave_equals`-Logik oder der `addbit()`-Funktion abgelehnt wird und das Paket somit nicht erfolgreich dekodiert wird. Es werden keine `0` oder `1` Bits geloggt, noch ein `X` für einen harten Mismatch.
    *   **Neue Erkenntnis:** Die `!r... f...` Debug-Ausgaben zeigen jetzt realistische `hightime` und `lowtime` Werte (z.B. `!r387 f409`, ~400µs), was bestätigt, dass die Timer-Implementierung und Mikrosekunden-Erfassung korrekt funktioniert. Die ISRs werden auch korrekt gezählt (`ISR H/L/T`).
...
*   **Weitere Entwicklung:**
    *   **Priorität 1: Tiefgehende Analyse der Protokoll-Dekodierung für SlowRF-Protokolle (Fokus auf `STATE_COLLECT` Abbruch).**
        *   **Ursache für `STATE_COLLECT` Abbruch:** Untersuchen, warum die Bit-Sammlung nach dem Sync sofort abbricht (`C<...>.R`) ohne Bit-Logs oder `X`-Fehler. Hierfür sind `rf_receive.c` (insbesondere `CC1100_in_callback`, `wave_equals`, `addbit`) und `rf_receive_bucket.c` zu prüfen.
        ...
        *   **`addbit`/`delbit` Logik:** Verifizieren, dass die Bits korrekt in den Bucket geschrieben werden und nicht fälschlicherweise entfernt werden oder die Logik übersprungen wird.
        *   **Deep-ISR-Logging:** Hinzufügen von minimalistischen Debug-Prints direkt in den `STATE_COLLECT`-Zweig der ISR, um die exakten `hightime`/`lowtime` Werte der ersten Datenbits nach dem Sync zu sehen.
        *   **State-Tracking:** Überprüfen, ob `b->state` durch einen Nebeneffekt (z. B. im `RfAnalyze_Task`) verändert wird, bevor das erste Bit verarbeitet werden kann.
