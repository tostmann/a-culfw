...
*   **XIAO-ESP32-C3 Target (Builds successful with safe pinout. Hardware boot-problem with CC1101 still requires verification):**
    *   `XIAO-ESP32-C3-factory.bin` (Vollständiges Image für `0x0` - **Erfolgreich erstellt mit sicherem Pinout**)
    *   `XIAO-ESP32-C3.bin` (Nur Applikation für OTA - **Erfolgreich erstellt mit sicherem Pinout**)
...
*   **PlatformIO Build-System-Fixes & Path Standardization für ESP32-Targets (DONE):**
    *   Die globale `src_dir = .` Einstellung wurde in die `[platformio]` Sektion verschoben, um das Projekt-Root-Verzeichnis für alle Targets als Quelle zu definieren.
    *   Alle `build_flags` für Include-Pfade (`-I`) und `build_src_filter` wurden für die ESP32-Targets (ESP32-C3, XIAO-ESP32-C3, ESP32-C6) vereinheitlicht und auf korrekte relative Pfade (z.B. `./`, `../../`, `../../clib`, `../../ESP32`) angepasst, um Build-Fehler zu beheben und die Konsistenz der Quellcode-Erkennung zu gewährleisten.
    *   Alle ESP32-Targets (C3, C6, XIAO-C3) bauen nun erfolgreich und generieren sowohl `factory.bin` als auch `app.bin`.
...
*   **XIAO-ESP32-C3 Boot-Problem mit CC1101 (Hardware-Verifikation ausstehend):**
    ...
    *   **Nächste Schritte (Manuelle Hardware-Verifikation UNBEDINGT erforderlich):**
        1.  **Flashen des Images:** Flashe das erfolgreich erstellte `XIAO-ESP32-C3-factory.bin` auf das Board.
