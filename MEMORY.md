...
*   **GDO-Interrupt-Handling & SlowRF-Puls-Analyse für ESP32 (DONE - WDT-Timeout behoben, präzises Timing):**
    *   **Problem (ehemals):** Ein Interrupt Watchdog Timeout auf CPU0 trat bei SlowRF-Verarbeitung auf, da die ISR zu lange lief.
    *   **Fix:** Der GDO0-Interrupt (`gdo_interrupt_handler`) wurde minimalistisch gehalten (`IRAM_ATTR` für minimale Latenz) und inkrementiert primär einen Zähler. Die zeitkritische Erfassung der Pulsbreiten erfolgt nun präzise mittels `esp_timer_get_time()` in der ISR (`CC1100_in_callback`), während die aufwendigere Analyse (`RfAnalyze_Task`) in der Hauptschleife (`loop()`) ausgeführt wird. Ein separater `rf_timer` überwacht die Funkstille. Dies verhindert Watchdog-Timeouts.
...
*   **IRAM_ATTR Optimierung (DONE):** Kritische Funktionen wie `addbit`, `makeavg` in `rf_receive_bucket.c` und `CC1100_in_callback` in `hal.cpp` wurden mit `IRAM_ATTR` markiert, um Latenzen durch Flash-Zugriffe während der Interrupt-Verarbeitung zu minimieren.
...
*   **Weitere Entwicklung:**
    *   **Priorität 1: Optimierung der SlowRF-Dekodierung:** Analyse der Toleranzschwellen (`TDIFF`) und Dekodierungslogik in `rf_receive.c`, um die Umwandlung der erkannten Pulse in vollständige Datenpakete für FS20 und Intertechno zu stabilisieren.
